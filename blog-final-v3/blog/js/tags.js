...
riot.tag2('editor', '<input class="u-full-width title" type="text" ref="post_title" placeholder="Post Title" value="{post ? post.title : \'\'}"> <div class="row"> <input class="six columns" type="text" ref="post_author" placeholder="Author" value="{post ? post.author : \'\'}"> <input class="six columns" type="text" ref="post_category" placeholder="Category" value="{post ? post.category : \'\'}"> </div> <textarea ref="post_content" class="u-full-width" placeholder="Post Content">{post ? post.content : \'\'}</textarea> <div class="row"> <button if="{post}" onclick="{delete_post}">Delete Post</button> <input onclick="{add_post}" type="submit" class="button button-primary u-pull-right" value="{opts.post ? \'Update\' :       \'Publish\'}"> </div>', 'editor .title,[riot-tag="editor"] .title,[data-is="editor"] .title{ height: 75px; font-size: 36px; } editor textarea,[riot-tag="editor"] textarea,[data-is="editor"] textarea{ height: 300px; max-width: 100%; min-width: 100%; }', '', function(opts) {
